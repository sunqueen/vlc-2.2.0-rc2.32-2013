...
void addons_manager_Gather( addons_manager_t *p_manager, const char *psz_uri )
{
    ...
    ARRAY_APPEND( (char **), p_manager->p_priv->finder.uris, strdup( psz_uri ) );			// sunqueen modify
                          ...
                          addon_entry_t **pp_addons, int i_count )
{
    ...
    for ( int i=0; i < i_count; i++ )
    {
        ...
        if ( !p_manager_entry )
        {
			ARRAY_APPEND((addon_entry_t **), p_manager->p_priv->finder.entries, p_entry);			// sunqueen modify
            vlc_event_t event;
...
static void *FinderThread( void *p_data )
{
    ...
    for( ;; )
    {
		...
		ARRAY_REMOVE((char **), p_manager->p_priv->finder.uris, 0);			// sunqueen modify
        vlc_cleanup_run();
...
static void *InstallerThread( void *p_data )
{
    ...
    for( ;; )
    {
        ...
        ARRAY_REMOVE( (addon_entry_t **), p_manager->p_priv->installer.entries, 0 );			// sunqueen modify
...
static int InstallEntry( addons_manager_t *p_manager, addon_entry_t *p_entry )
{
    ...
    vlc_mutex_lock( &p_manager->p_priv->installer.lock );
    ARRAY_APPEND( (addon_entry_t **), p_manager->p_priv->installer.entries, p_entry );			// sunqueen modify
