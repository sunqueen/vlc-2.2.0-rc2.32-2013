...
#   endif
	...
	vgl->subpicture_buffer_object = (GLuint *)malloc(subpicture_buffer_object_count * sizeof(GLuint));			// sunqueen modify
...
static void orientationTransformMatrix(GLfloat matrix[/*static*/ 16], video_orientation_t orientation) {			// sunqueen modify
                                ...
                                const video_format_t *source)
{
    ...
    if (2 * vgl->region_count > vgl->subpicture_buffer_object_count) {
		...
		vgl->subpicture_buffer_object = (GLuint *)realloc_or_free(vgl->subpicture_buffer_object, new_count * sizeof(GLuint));			// sunqueen modify
        if (!vgl->subpicture_buffer_object) {
            vlc_gl_Unlock(vgl->gl);
            return VLC_ENOMEM;
        }
