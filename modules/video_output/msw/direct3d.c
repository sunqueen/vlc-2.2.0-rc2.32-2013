...
#define ID3DXBuffer_GetBufferPointer(p)   (p)->GetBufferPointer()			// sunqueen add
...
static int Direct3DCreatePool(vout_display_t *vd, video_format_t *fmt)
{
    ...
    *fmt = vd->source;
	...
	picture_resource_t resource = { /*.p_sys =*/ picsys };			// sunqueen modify
...
static int Direct3DCompileShader(vout_display_t *vd, const char *shader_source, size_t source_length)
{
	...
	OurD3DXCompileShader = (HRESULT(__stdcall *)(LPCSTR, UINT, const D3DXMACRO *, LPD3DXINCLUDE, LPCSTR, LPCSTR, DWORD, LPD3DXBUFFER *, LPD3DXBUFFER *, LPD3DXCONSTANTTABLE *))GetProcAddress(sys->hd3d9x_dll, "D3DXCompileShader");			// sunqueen modify
    ...
    hr = IDirect3DDevice9_CreatePixelShader(sys->d3ddev,
            (const DWORD *) ID3DXBuffer_GetBufferPointer(compiled_shader),			// sunqueen modify
            &sys->d3dx_shader);
...
static int Direct3DCreateShaders(vout_display_t *vd)
{
    ...
    if (shader_source_builtin) {
        ...
        if (length == -1 || length >= MAX_SHADER_FILE_SIZE)
            goto error;
        rewind(fs);
		shader_source_file = (char *)malloc(sizeof(*shader_source_file) * length);			// sunqueen modify
 ...
 */
static void orientationVertexOrder(video_orientation_t orientation, int vertex_order[/*static*/ 4])			// sunqueen modify
{
    switch (orientation) {
...
static void ListShaders(enum_context_t *ctx)
{
	...
	ctx->values = (char **)xrealloc(ctx->values, (ctx->count + num_shaders + 1) * sizeof(char *));			// sunqueen modify
	ctx->descs = (char **)xrealloc(ctx->descs, (ctx->count + num_shaders + 1) * sizeof(char *));			// sunqueen modify
