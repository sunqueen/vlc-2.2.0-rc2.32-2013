                ...
                *ppp_title = malloc( sizeof( input_title_t* ) );
                ...
                for( int i = 0; i < p_sys->i_seekpoints; i++ )
                {
                    ...
                    if ( likely( p_seekpoint_copy ) )
                        TAB_APPEND( (seekpoint_t **), p_title->i_seekpoint, p_title->seekpoint, p_seekpoint_copy );			// sunqueen modify
                }
                *pi_title_offset = 0;
    ...
    while ( p <= p_boundary - 1 && p > p_oggpacket->packet )
    {
        if ( *p == 0x0D && *(p+1) == 0x0A )
        {
            ...
            if ( psz_message )
            {
                ...
                TAB_APPEND( (char **), p_skel->i_messages, p_skel->ppsz_messages, psz_message );			// sunqueen modify
            }
