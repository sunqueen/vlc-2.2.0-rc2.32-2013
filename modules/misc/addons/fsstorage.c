...
//#include <unistd.h>     // getpid()			// sunqueen delete
...
static int ListSkins( addons_finder_t *p_finder )
{
    ...
    for ( int i=0; i< i_count; i++ )
    {
        ...
        if( asprintf( &psz_uri, "unzip://%s/%s!/theme.xml", psz_dir, psz_file ) >= 0)
        {
            ...
            ARRAY_APPEND( (addon_entry_t **), p_finder->entries, p_entry );			// sunqueen modify
        }
        free( psz_file );
    }
...
//            char psz_parent[strlen( psz_dirname ) + 1], *psz_end;
			char *psz_parent, *psz_end;

			psz_parent = (char *)malloc(sizeof(char *) * (strlen(psz_dirname) + 1));
			// sunqueen modify end
        ...
        case XML_READER_ENDELEM:
            ...
            if ( ! strcmp( p_node, "resource" ) )
            {
                if ( p_entry && psz_filename && i_filetype >= 0 )
                {
                    ...
                    ARRAY_APPEND( (addon_file_t **), p_entry->files, p_file );			// sunqueen modify
                }
                /* reset temp */
                psz_filename = NULL;
                i_filetype = -1;
