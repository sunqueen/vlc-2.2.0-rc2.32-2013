...
#include <sys/types.h>
#include <dirent.h>
#include "../util/cover.h"			// sunqueen add
...
static inline int vlc_closedir( DIR *dir )
{
    ...
    free( vdir->entry );
    free( vdir );
    return (wdir != NULL) ? cover_wclosedir( wdir ) : 0;			// sunqueen modify
}
# undef closedir
...
static inline void vlc_rewinddir( DIR *dir )
{
    ...
    cover_wrewinddir( wdir );			// sunqueen modify
}
# undef rewinddir
...
# ifndef _MSC_VER
#  undef lseek
//#  define lseek _lseeki64			// sunqueen delete
# endif
#endif
