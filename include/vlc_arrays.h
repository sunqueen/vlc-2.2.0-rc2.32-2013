 ...
 */
#define INSERT_ELEM( cast, p_ar, i_oldsize, i_pos, elem )                           \
    do                                                                        \
    {                                                                         \
        if( !(i_oldsize) ) (p_ar) = NULL;                                       \
        (p_ar) = cast realloc( p_ar, ((i_oldsize) + 1) * sizeof(*(p_ar)) ); \
        if( !(p_ar) ) abort();                                                \
        if( (i_oldsize) - (i_pos) )                                           \
        {                                                                     \
...
#define REMOVE_ELEM( cast, p_ar, i_size, i_pos )                                    \
    do                                                                        \
    {                                                                         \
        if( (i_size) - (i_pos) - 1 )                                          \
        {                                                                     \
        ...
        if( i_size > 1 )                                                      \
            (p_ar) = cast realloc_down( p_ar, ((i_size) - 1) * sizeof( *(p_ar) ) );\
        else                                                                  \
        {                                                                     \
            free( p_ar );                                                     \
            (p_ar) = NULL;                                                    \
...
#define TAB_APPEND( cast, count, tab, p )             \
    TAB_APPEND_CAST( cast, count, tab, p )
...
#define TAB_INSERT( cast, count, tab, p, index )      \
    TAB_INSERT_CAST( cast, count, tab, p, index )
...
/* Internal functions */
#define _ARRAY_ALLOC(cast, array, newsize) {                                      \
    (array).i_alloc = newsize;                                              \
    (array).p_elems = cast realloc( (array).p_elems, (array).i_alloc *    \
                               sizeof(*(array).p_elems) );                  \
    if( !(array).p_elems ) abort();                                         \
}

#define _ARRAY_GROW1(cast, array) {                                               \
    if( (array).i_alloc < 10 )                                              \
        _ARRAY_ALLOC(cast, array, 10 )                                            \
    else if( (array).i_alloc == (array).i_size )                            \
        _ARRAY_ALLOC(cast, array, (int)(array.i_alloc * 1.5) )                    \
}
...
#define ARRAY_APPEND(cast, array, elem)                                           \
  do {                                                                      \
    _ARRAY_GROW1(cast, array);                                                    \
    (array).p_elems[(array).i_size] = elem;                                 \
    (array).i_size++;                                                       \
  } while(0)

#define ARRAY_INSERT(cast, array,elem,pos)                                        \
  do {                                                                      \
    _ARRAY_GROW1(cast, array);                                                    \
    if( (array).i_size - pos ) {                                            \
...
#define _ARRAY_SHRINK(cast, array) {                                              \
    if( (array).i_size > 10 && (array).i_size < (int)((array).i_alloc / 1.5) ) {  \
        _ARRAY_ALLOC(cast, array, (array).i_size + 5);                            \
    }                                                                       \
}

#define ARRAY_REMOVE(cast, array, pos)                                             \
  do {                                                                      \
